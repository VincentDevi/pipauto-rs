//! Bounded Month and Week calendar model query.
...
use crate::{
    ...
    models::{
        calendar::{CalendarEntry, CalendarRange, CalendarView},
        intervention::calendar_repository::CalendarRepository,
    },
};
...
mod tests {
    ...
    use super::*;
    use crate::{domain::Clock, models::persistence_error::PersistenceError as RepositoryError};
    ...
    fn service(now: &str) -> (CalendarModel, Arc<RecordingRepository>) {
        let repository = Arc::new(RecordingRepository::default());
        let time = WorkshopTime::new(
            chrono_tz::Europe::Brussels,
            Arc::new(FixedClock(instant(now))),
        );
        (CalendarModel::new(repository.clone(), time), repository)
    }
