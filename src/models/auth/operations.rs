//! Authentication model operations.
...
impl AuthenticationModel {
    /// Compose model-owned authentication persistence with explicit external capabilities.
    ...
    pub fn from_infrastructure(
        ...
        let repository = Arc::new(super::persistence::SurrealAuthRepository::new(client));
        Self::new(
            settings,
            repository.clone(),
            repository.clone(),
...
mod tests {
    ...
    async fn auth_service_login_persists_before_jwt_and_revokes_if_issuance_fails() {
        ...
        let service = AuthenticationModel::new(
            settings,
            repository.clone(),
            repository.clone(),
            repository,
